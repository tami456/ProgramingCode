...
void KeyconfigScene::CreateCommnadStringList(const InputTable_t& table)
{
	commandStringList_.clear();
	for (const auto& record : table)
	{
		commandStringList_.push_back(
			std::make_pair(record.first,
...
KeyconfigScene::KeyconfigScene(SceneManager& manager):Scene(manager)
{
	...
	if (padType == DX_PADTYPE_XBOX_360 || padType == DX_PADTYPE_XBOX_ONE)
	{
		...
		padNameMap_[PAD_INPUT_START] = L"L3ボタン";
		padNameMap_[PAD_INPUT_M] = L"R3ボタン";
	}
	else
	{
		padNameMap_[PAD_INPUT_A] = L"Ａボタン";
...
void KeyconfigScene::Update(Input& input)
{
	...
	{
		inputTable_ = input.GetInputTabel();
		CreateCommnadStringList(inputTable_);
	}
...
void KeyconfigScene::Draw()
{
	...
	for (const auto& pair : commandStringList_)
	{
		...
		int commandStringColor_ = 0xffffff;
		if (cursorIndex_ == count)
		{
			...
			{
				isEditting = true;
				commandStringColor_ = 0xffbb88;
			}
		}
		++count;

		
		//DrawFormatStringのためのワイド文字列に変換する
		//なので、ベクタをwstringにしておく
		DrawFormatString(x, y, commandStringColor_,
	...
	DrawFormatString(200, 300, 0xffffff, 
		L"Analog x = %d, y = %d",analogX,analogY);
	...
	DrawFormatString(200, 450, 0xffffff,
		L"RightLever 状態=%s", inputStr.c_str());
	DrawFormatString(200, 400, 0xffffff,
		L"Trigget L = %d, R = %d", xstate.LeftTrigger, xstate.RightTrigger);
}
